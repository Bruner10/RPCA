...
%{
  Didn't get a very good Low Rank Approximation but it was a lot better than the corrupted image
...
% Vectorize the images into a single Design Matrix
Z = [B(:), C(:), D(:), E(:), M(:)];
...
% Unvectorize the returned matrices to get the individual images back
B = reshape(L(:,1), x, y);
C = reshape(L(:,2), x, y);
D = reshape(L(:,3), x, y);
E = reshape(L(:,4), x, y);
